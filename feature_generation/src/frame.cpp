...
public:
    ...
    float getHorzDistanceBwCentroids(const ObjectProfile & other) const {
        return sqrt(pow(centroid.x - other.centroid.x, 2) + pow(centroid.y - other.centroid.y, 2));
    }

    float getDistanceSqrBwCentroids(const ObjectProfile & other) const {
        return pow(centroid.x - other.centroid.x, 2) + pow(centroid.y - other.centroid.y, 2) + pow(centroid.z - other.centroid.z, 2);
    }
    float getDistanceSqrBwCenters(const ObjectProfile & other) const {
...
public:
    ...
    Frame(int ***IMAGE, double** data, double **pos_data, vector<vector<double> > &objFeats, string seqId, int fnum, string transformfile) {
        ...
        savePointCloud();
        skeleton = new Frame_skel(data, pos_data, transformfile);
     ...
     Frame(int ***IMAGE, double** data, double **pos_data, vector<vector<double> > &objFeats, string seqId, int fnum) {
        ...
        //savePointCloud();
        skeleton = new Frame_skel(data, pos_data);
