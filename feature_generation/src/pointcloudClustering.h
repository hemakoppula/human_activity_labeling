...
typedef pcl::KdTree<PointOutT> KdTree;
typedef pcl::KdTree<PointOutT>::Ptr KdTreePtr;
...
int getClusters(pcl::PointCloud<PointOutT> &cloud, std::vector<pcl::PointCloud<PointT> > &clustersOut, std::vector<pcl::PointIndices> &clusterInds, bool useNormals) {
    ...
    float radius = 0.1;//0.025; // 0.01*10 ;//0.025*1000;//0.01*1000;// 0.025 
...
void computeCentroid(geometry_msgs::Point32 &centroid, pcl::PointCloud<PointT> &cloud) {
...
void getNormal(pcl::PointCloud<PointT> &cloud, Eigen::Vector3d &normal) {
    ...
    computeCovarianceMatrix(cloudMsg, covariance_matrix,centroid);
...
void filterForTable(pcl::PointCloud<PointOutT> &cloud, pcl::PointIndices &tablePoints) {
    ...
    for (int i = 0; i < cloud.size(); i++) {
        ...
        if (cloud.points[i].distance < 3000 && cloud.points[i].distance > 0 && cloud.points[i].z < 100 && cloud.points[i].z > -100) {
