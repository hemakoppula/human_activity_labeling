		      ...
		      STRUCTMODEL *sm)
{
  ...
  do { /* iteratively increase precision */
    ...
    print_struct_iteration_stats(ceps,0,sample,sm,cset,alpha,sparm);
  ...
  if(struct_verbosity>=1) {
    ...
    int numslacks=n;
    for(i=0; i<cset.m; i++) {
      numslacks = (numslacks > cset.lhs[i]->slackid) ?
	numslacks : cset.lhs[i]->slackid;
    }
    slacks=(double *)my_malloc(sizeof(double)*(numslacks+1));
    for(i=0; i<=numslacks; i++) { 
      slacks[i]=0;
    }
    if(sparm->slack_norm == 1) {
      for(j=0;j<cset.m;j++) 
  ...
  */
  ...
  do { /* iteratively find and add constraints to working set */
	...
	printf("(NumConst=%d, SV=%ld, CEps=%.4f, QPEps=%.4f)\n",cset.m,
      ...
      print_struct_iteration_stats(ceps,cached_constraint,sample,sm,cset,
				   alpha,sparm);
